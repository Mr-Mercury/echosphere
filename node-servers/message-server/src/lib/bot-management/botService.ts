...
export class BotServiceManager {
    ...
    private deactivateBotInMemory(botId: string): boolean {
    ...
    private async generateMessage(config: BotConfiguration, channelId: string, channelName: string): Promise<ProcessedMessage | null> {
        try {
    ...
    public async stopAllServerBots(serverId: string) {
        ...
        const serverBotInstances = Array.from(this.bots.values()).filter(
            botInstance => botInstance.config.homeServerId === serverId
        );

        const serverBotIds = serverBotInstances.map(instance => instance.config.id);
        
        interface BotResult { id: string; name: string; }
        interface BotErrorResult extends BotResult { error: string; }
        const results: { successful: BotResult[]; failed: BotErrorResult[]; } = { successful: [], failed: [] };
        ...
        try {
            ...
            for (const botInstance of serverBotInstances) {
                try {
                    ...
                    results.failed.push({ // Record as failed initially, but attempted cleanup
                        id: botInstance.config.id, 
                        name: botInstance.config.botName, 
