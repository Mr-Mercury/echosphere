...
function parseChannelsPreview(json) {
    let parsedJson;
    ...
    if (Array.isArray(parsedJson)) {
        return parsedJson.filter((item) => item &&
...
function parseTags(json) {
    ...
    if (Array.isArray(parsedJson)) {
        return parsedJson.filter((item) => typeof item === 'string');
    }
    return [];
}
...
export function calculateServerPopularityScore(usageCount) {
...
async function fetchPopularServerTemplatesImpl() {
    ...
    const transformedServers = topServerTemplates.map(st => ({
        id: st.id,
        name: st.serverName,
 ...
 */
export const fetchPopularServerTemplates = async () => {
    try {
 ...
 */
export const fetchPopularServerTemplatesWithRevalidation = unstable_cache(async () => {
...
export async function fetchServerTemplatesWithFilters({ page, pageSize, sort = 'popular', category, searchQuery, creatorId, isPublic = true, // Default to fetching public templates
 }) {
    try {
        const where = { isPublic }; // Initialize with isPublic
        if (creatorId) {
        ...
        const transformedServers = serverTemplatesData.map(st => ({
            id: st.id,
            name: st.serverName,
            description: st.description || '',
...
export const fetchServerTemplatesWithFiltersCached = unstable_cache(async (params) => {
