...
export function calculatePopularityScore(copiesCreated, likes, dislikes) {
...
async function fetchPopularBotTemplatesImpl() {
    ...
    const transformedBots = topBots.map(bot => ({
        id: bot.id,
        name: bot.botName,
 ...
 */
export const fetchPopularBotTemplates = async () => {
    try {
 ...
 */
export const fetchPopularBotTemplatesWithRevalidation = unstable_cache(async () => {
    console.log('Popular bots cache revalidated');
    return await fetchPopularBotTemplates();
}, ['popular-bot-templates'], { revalidate: CACHE_CONSTANTS.POPULAR_BOTS });
// Fetch bot templates with filtering, sorting, and pagination
export async function fetchBotTemplatesWithFilters({ page, pageSize, sort = 'popular', model, searchQuery, creatorId }) {
    try {
        ...
        if (model && model !== 'All Models') {
            const modelFamilyInfo = MODEL_DISPLAY[model];
            if (modelFamilyInfo && modelFamilyInfo.provider) {
                ...
                const modelIdsInFamily = Object.keys(AVAILABLE_MODELS).filter(key => AVAILABLE_MODELS[key].provider === targetProvider);
...
export const fetchBotTemplatesWithFiltersCached = unstable_cache(async (params) => {
    console.log('Bot templates cache revalidated for params:', params);
    return await fetchBotTemplatesWithFilters(params);
}, ['bot-templates-filtered'], { revalidate: CACHE_CONSTANTS.BOT_TEMPLATES });
//# sourceMappingURL=botTemplates.js.map